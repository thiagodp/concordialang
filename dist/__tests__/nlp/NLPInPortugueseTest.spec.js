    ...
    describe('testcase entities', () => {
        ...
        describe('free', () => {
            ...
            it('see with long escaped css selector as ui literal', () => {
                ...
                shouldHaveTestCaseEntities(results, [UI_ACTION, UI_LITERAL]);
                expect(r.entities.find(e => e.entity === UI_LITERAL).value).toEqual('#js-repo-pjax-container > div.container.new-discussion-timeline.experiment-repo-nav > div.repository-content > div.release-show > div > div.release-body.commit.open.float-left > div.my-4 > h2');
            });
            it('drag and drop with with long escaped css selector as ui literals', () => {
                let results = [];
                let r;
                results.push(r = recognizeInTestCase('eu arrasto <#js-repo-pjax-container \> div.container \> div h2> para <foo \> bar \> zoo>'));
            ...
            it('fill with ui literal and problematic values', () => {
                let results = [];
                let r;
                results.push(r = recognizeInTestCase('Quando eu informo <#foo> com    "t-HK pf79U~M#Q?mDTN~eW;+UwsheU8^@_j2\\qaX6;w\\""'));
                shouldHaveTestCaseEntities(results, [UI_LITERAL, VALUE]);
                let uiLiterals = r.entities.filter(e => e.entity === UI_LITERAL);
                let values = r.entities.filter(e => e.entity === VALUE);
            ...
            it('fill with ui literal and long negative number', () => {
                let results = [];
                let r;
                results.push(r = recognizeInTestCase('Quando eu preencho <bar> com -53358731722743'));
