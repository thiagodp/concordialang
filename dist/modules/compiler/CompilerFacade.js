...
export function filterFilesToCompile(files, extensionFeature, extensionTestCase) {
    ...
    const testCasesWithoutFeature = onlyTestCases
        .filter(tc => !featureFiles.includes(toUnixPath(changeFileExtension(tc, extensionFeature))));
...
export class CompilerFacade {
    ...
    async compile(options) {
        ...
        const lexer = new Lexer(options.language, languageMap);
        ...
        if (this._compilerListener && compiledFilesCount) {
            const durationMS = Date.now() - startTime;
            const testCasesCount = output.spec?.docs?.filter(doc => doc.fileInfo?.path?.endsWith(options.extensionTestCase)).length;
        ...
        const tcGen = new TestCaseGeneratorFacade(nlpBasedSentenceRecognizer.variantSentenceRec, languageMap, this._tcGenListener, fileHandler);
        return await tcGen.execute(options, output.spec, output.graph);
    }
    updateSeed(options, ui) {
        if (!options.seed) {
            options.isGeneratedSeed = true;
            options.seed = LocalDateTime.now().format(DateTimeFormatter.ofPattern('yyyy-MM-dd HH:mm:ss')).toString();
        }
