...
import { makePackageInitCommand, makePackageInstallCommand, makePackageUninstallCommand, } from '../util/package-installation';
import { runCommand } from '../util/run-command';
import { PACKAGE_FILE, PLUGIN_PREFIX } from './PluginData';
...
export class PluginManager {
    ...
    async pluginWithName(name, partialComparison = false) {
    ...
    async installByName(name) {
        ...
        if (pluginData) { // already exists
            ...
            if (mustGeneratePackageFile) {
                const cmd = makePackageInitCommand(this._packageManager); // 'npm init --yes';
                await this.runCommand(cmd);
            }
        }
        // Install the package as a DEVELOPMENT dependency
        const command = makePackageInstallCommand(name, this._packageManager);
    ...
    async uninstallByName(name) {
        ...
        const command = makePackageUninstallCommand(name, this._packageManager);
