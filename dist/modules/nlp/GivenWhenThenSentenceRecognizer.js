...
class GivenWhenThenSentenceRecognizer {
    ...
    recognizeSentences(language, nodes, errors, warnings, ownerName = 'Variant') {
        ...
        let processor = function (node, r, errors, warnings) {
            ...
            const actionIndex = recognizedEntityNames.indexOf(concordialang_types_1.Entities.UI_ACTION);
            const execActionIndex = recognizedEntityNames.indexOf(concordialang_types_1.Entities.EXEC_ACTION);
            ...
            item.action = action;
            // modifier (optional)
            const modifiers = r.entities.filter(e => e.entity === concordialang_types_1.Entities.UI_ACTION_MODIFIER).map(e => e.value);
            if (modifiers.length > 0) {
                item.actionModifier = modifiers[0];
            }
            // options (optional)
            const options = r.entities.filter(e => e.entity === concordialang_types_1.Entities.UI_ACTION_OPTION).map(e => e.value);
            if (options.length > 0) {
                item.actionOptions = options;
            }
            // targets - UI LITERALS (optional)
            item.targets = r.entities.filter(e => e.entity === concordialang_types_1.Entities.UI_LITERAL).map(e => e.value);
            // target types
            item.targetTypes = r.entities.filter(e => e.entity === concordialang_types_1.Entities.UI_ELEMENT_TYPE).map(e => e.value);
            // values (optional)
            ...
            let commands = r.entities.filter(e => e.entity === concordialang_types_1.Entities.COMMAND)
